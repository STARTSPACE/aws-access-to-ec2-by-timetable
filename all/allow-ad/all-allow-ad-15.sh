...
sudo aws ec2 describe-security-groups --filters Name=tag:open-allports-time,Values=15-00 Name=tag:bash-profile,Values=ad --query SecurityGroups[].[GroupId] --output text > ~/tmp/allowall_ad_info.txt 2>&1

# Take list of changing security groups
for group_id in $(cat ~/tmp/allowall_ad_info.txt)
