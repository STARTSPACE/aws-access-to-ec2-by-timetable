...
sudo aws ec2 describe-security-groups --filters Name=tag:bash-allowall,Values=05-00 Name=tag:bash-profile,Values=ad --query SecurityGroups[].[GroupId] --output text > ~/tmp/allowall_ad_info.txt 2>&1
