...
sudo aws ec2 describe-security-groups --filters Name=tag:bash-allowall,Values=05-00 Name=tag:bash-profile,Values=wd --query SecurityGroups[].[GroupId] --output text > ~/tmp/allowall_wd_info.txt 2>&1
