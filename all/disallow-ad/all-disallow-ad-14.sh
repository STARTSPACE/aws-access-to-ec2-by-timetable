...
sudo aws ec2 describe-security-groups --filters Name=tag:bash-disallowall,Values=14-00 Name=tag:bash-profile,Values=ad --query SecurityGroups[].[GroupId] --output text > ~/tmp/disallowall_ad_info.txt 2>&1
