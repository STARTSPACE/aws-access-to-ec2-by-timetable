...
sudo aws ec2 describe-security-groups --filters Name=tag:bash-disallowall,Values=14-00 Name=tag:bash-profile,Values=wd --query SecurityGroups[].[GroupId] --output text > ~/tmp/disallowall_wd_info.txt 2>&1
