...
sudo aws ec2 describe-security-groups --filters Name=tag:bash-disallowssh,Values=04-00 Name=tag:bash-profile,Values=ad --query SecurityGroups[].[GroupId] --output text > ~/tmp/disallowssh_ad_info.txt 2>&1

# Take list of changing security groups
for group_id in $(cat ~/tmp/disallowssh_ad_info.txt)
