...
sudo aws ec2 describe-security-groups --filters Name=tag:bash-disallowssh,Values=11-00 Name=tag:bash-profile,Values=wd --query SecurityGroups[].[GroupId] --output text > ~/tmp/disallowssh_wd_info.txt 2>&1
