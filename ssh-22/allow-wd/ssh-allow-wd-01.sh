...
sudo aws ec2 describe-security-groups --filters Name=tag:bash-allowssh,Values=01-00 Name=tag:bash-profile,Values=wd --query SecurityGroups[].[GroupId] --output text > ~/tmp/allowssh_wd_info.txt 2>&1

# Take list of changing security groups
for group_id in $(cat ~/tmp/allowssh_wd_info.txt)
