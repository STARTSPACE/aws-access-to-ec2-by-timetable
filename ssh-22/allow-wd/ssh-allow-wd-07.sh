...
sudo aws ec2 describe-security-groups --filters Name=tag:bash-allowssh,Values=07-00 Name=tag:bash-profile,Values=wd --query SecurityGroups[].[GroupId] --output text > ~/tmp/allowssh_wd_info.txt 2>&1
