...
sudo aws ec2 describe-security-groups --filters Name=tag:bash-allowhttps,Values=18-00 Name=tag:bash-profile,Values=ad --query SecurityGroups[].[GroupId] --output text > ~/tmp/allowhttps_ad_info.txt 2>&1
