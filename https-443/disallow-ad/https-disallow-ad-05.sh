...
sudo aws ec2 describe-security-groups --filters Name=tag:bash-disallowhttps,Values=05-00 Name=tag:bash-profile,Values=ad --query SecurityGroups[].[GroupId] --output text > ~/tmp/disallowhttps_ad_info.txt 2>&1

# Take list of changing security groups
for group_id in $(cat ~/tmp/disallowhttps_ad_info.txt)
