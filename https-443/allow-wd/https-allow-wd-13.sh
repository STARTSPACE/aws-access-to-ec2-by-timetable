...
sudo aws ec2 describe-security-groups --filters Name=tag:open-https-time,Values=13-00 Name=tag:bash-profile,Values=wd --query SecurityGroups[].[GroupId] --output text > ~/tmp/allowhttps_wd_info.txt 2>&1

# Take list of changing security groups
for group_id in $(cat ~/tmp/allowhttps_wd_info.txt)
