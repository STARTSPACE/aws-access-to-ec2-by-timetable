...
sudo aws ec2 describe-security-groups --filters Name=tag:bash-disallowhttps,Values=07-00 Name=tag:bash-profile,Values=wd --query SecurityGroups[].[GroupId] --output text > ~/tmp/disallowhttps_wd_info.txt 2>&1
