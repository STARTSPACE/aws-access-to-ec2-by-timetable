...
sudo aws ec2 describe-security-groups --filters Name=tag:bash-disallowhttp,Values=18-00 Name=tag:bash-profile,Values=wd --query SecurityGroups[].[GroupId] --output text > ~/tmp/disallowhttp_wd_info.txt 2>&1

# Take list of changing security groups
for group_id in $(cat ~/tmp/disallowhttp_wd_info.txt)
