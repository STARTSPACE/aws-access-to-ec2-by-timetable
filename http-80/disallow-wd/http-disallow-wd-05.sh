...
sudo aws ec2 describe-security-groups --filters Name=tag:bash-disallowhttp,Values=05-00 Name=tag:bash-profile,Values=wd --query SecurityGroups[].[GroupId] --output text > ~/tmp/disallowhttp_wd_info.txt 2>&1
