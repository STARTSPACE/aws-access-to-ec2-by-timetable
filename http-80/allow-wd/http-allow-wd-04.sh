...
sudo aws ec2 describe-security-groups --filters Name=tag:bash-allowhttp,Values=04-00 Name=tag:bash-profile,Values=wd --query SecurityGroups[].[GroupId] --output text > ~/tmp/allowhttp_wd_info.txt 2>&1
