...
sudo aws ec2 describe-security-groups --filters Name=tag:bash-allowhttp,Values=07-00 Name=tag:bash-profile,Values=ad --query SecurityGroups[].[GroupId] --output text > ~/tmp/allowhttp_ad_info.txt 2>&1
