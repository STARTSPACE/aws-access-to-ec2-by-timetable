...
sudo aws ec2 describe-security-groups --filters Name=tag:bash-disallowhttp,Values=11-00 Name=tag:bash-profile,Values=ad --query SecurityGroups[].[GroupId] --output text > ~/tmp/disallowhttp_ad_info.txt 2>&1
